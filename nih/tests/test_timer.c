...
test_add_timeout (void)
{
	...
	t1 = time (NULL);
	timer = nih_timer_add_timeout (my_callback, &t1, 10);
	t2 = time (NULL);
...
test_add_periodic (void)
{
	...
	t1 = time (NULL);
	timer = nih_timer_add_periodic (my_callback, &t1, 25);
	t2 = time (NULL);
...
test_add_scheduled (void)
{
	...
	t1 = time (NULL);
	timer = nih_timer_add_scheduled (my_callback, &t1, &schedule);
	t2 = time (NULL);
...
test_next_due (void)
{
	...
	timer1 = nih_timer_add_timeout (my_callback, &ret, 10);
	timer2 = nih_timer_add_timeout (my_callback, &ret, 5);
	timer3 = nih_timer_add_timeout (my_callback, &ret, 15);
...
test_poll (void)
{
	...
	timer1 = nih_timer_add_timeout (my_callback, &t1, 10);
	timer2 = nih_timer_add_periodic (my_callback, &t2, 20);
